...
class SimpleFOCDevice:
    ...
    def __init__(self):
        ...
        if SimpleFOCDevice.__instance != None:
            ...
            self.commandIoLock = threading.Lock()
            self.pullConfigurationLock = threading.Lock()
            self.isPullingConfiguration = False
            ...
            self.passiveTorqueFollowLowPidI = 0.0
            self.passiveTorqueFollowLowPidD = 0.0
            self.passiveTorqueFollowRunPidP = 0.08
            self.passiveTorqueFollowRunPidI = 0.0
            self.passiveTorqueFollowRunPidD = 0.0
            self.passiveTorqueFollowPidP = self.passiveTorqueFollowLowPidP
            self.passiveTorqueFollowPidI = self.passiveTorqueFollowLowPidI
            self.passiveTorqueFollowPidD = self.passiveTorqueFollowLowPidD
    ...
    def configureDevice(self, jsonValue):
        ...
        try:
            self.modulationType = int(jsonValue['modulationType'])
        except KeyError:
            pass
        try:
            self.modulationCentered = int(jsonValue['modulationCentered'])
        except KeyError:
            pass
        try:
            self.deviceStatus = int(jsonValue['deviceStatus'])
        except KeyError:
            pass
        try:
            self.releaseMode = bool(jsonValue['releaseMode'])
        except KeyError:
            pass
        try:
            self.passiveTorqueMode = bool(jsonValue['passiveTorqueMode'])
        except KeyError:
            pass
        try:
            self.passiveTorqueTargetNm = float(jsonValue['passiveTorqueTargetNm'])
        except KeyError:
            pass
        try:
        ...
        try:
            self.passiveTorqueFollowDeadzoneDeg = float(
                jsonValue['passiveTorqueFollowDeadzoneDeg'])
        except KeyError:
            pass
        try:
            self.passiveTorqueRunningSpeedThresholdRadS = float(
                jsonValue['passiveTorqueRunningSpeedThresholdRadS'])
        except KeyError:
            pass
        try:
        ...
        self.passiveTorqueUpdateHz = self.passiveTorqueCalculationHz
        try:
            self.passiveTorqueFollowLowPidP = float(
                jsonValue.get('passiveTorqueFollowLowPidP',
                              jsonValue['passiveTorqueFollowPidP']))
        except KeyError:
            pass
        try:
            self.passiveTorqueFollowLowPidI = float(
                jsonValue.get('passiveTorqueFollowLowPidI',
                              jsonValue['passiveTorqueFollowPidI']))
        except KeyError:
            pass
        try:
            self.passiveTorqueFollowLowPidD = float(
                jsonValue.get('passiveTorqueFollowLowPidD',
                              jsonValue['passiveTorqueFollowPidD']))
        except KeyError:
            pass
        try:
            self.passiveTorqueFollowRunPidP = float(
                jsonValue['passiveTorqueFollowRunPidP'])
        except KeyError:
            pass
        try:
            self.passiveTorqueFollowRunPidI = float(
                jsonValue['passiveTorqueFollowRunPidI'])
        except KeyError:
            pass
        try:
            self.passiveTorqueFollowRunPidD = float(
                jsonValue['passiveTorqueFollowRunPidD'])
        except KeyError:
            pass
        self.passiveTorqueFollowPidP = self.passiveTorqueFollowLowPidP
        self.passiveTorqueFollowPidI = self.passiveTorqueFollowLowPidI
        ...
        try:
            ...
            pass
        try:
            self.stateUpdateRateHz = max(1, int(jsonValue['stateUpdateRateHz']))
        except KeyError:
            pass
    ...
    def toJSON(self):
        valuesToSave = {
            ...
            'modulationCentered': self.modulationCentered,
            ...
            'passiveTorqueRunningSpeedThresholdRadS': self.passiveTorqueRunningSpeedThresholdRadS,
            'passiveTorqueCalculationHz': self.passiveTorqueCalculationHz,
            'passiveTorqueUpdateHz': self.passiveTorqueCalculationHz,
            'passiveTorqueFollowPidP': self.passiveTorqueFollowLowPidP,
            'passiveTorqueFollowPidI': self.passiveTorqueFollowLowPidI,
            'passiveTorqueFollowPidD': self.passiveTorqueFollowLowPidD,
            'passiveTorqueFollowLowPidP': self.passiveTorqueFollowLowPidP,
            'passiveTorqueFollowLowPidI': self.passiveTorqueFollowLowPidI,
            'passiveTorqueFollowLowPidD': self.passiveTorqueFollowLowPidD,
            'passiveTorqueFollowRunPidP': self.passiveTorqueFollowRunPidP,
            'passiveTorqueFollowRunPidI': self.passiveTorqueFollowRunPidI,
            'passiveTorqueFollowRunPidD': self.passiveTorqueFollowRunPidD,
    ...
    def toArduinoCode(self, generateParams = []):
        ...
        if generateParams[0] or generateParams==[]:
            ...
            if self.controlType == self.TORQUE_CONTROL:
                ...
                code += "angle"
            elif self.controlType == self.VELOCITY_OPENLOOP_CONTROL:
                code += "velocity_openloop"
            elif self.controlType == self.ANGLE_OPENLOOP_CONTROL:
                code += "angle_openloop"
            code += ";\n"
        ...
        if generateParams[8] or generateParams==[]:
            ...
            code += "motor.phase_resistance = " + str(self.phaseResistance) +";\n" 
    ...
    def setStateUpdateRateHz(self, value):
        self.stateUpdateRateHz = max(1, int(float(value)))

    def sendCommand(self, command):
        if self.isConnected:
            try:
                with self.commandIoLock:
                    self.serialPort.write((str(command) + '\n').encode('utf-8'))
            except (SerialException, SerialTimeoutException) as error:
                logging.warning('Serial write failed for command %s', command, exc_info=True)
    ...
    def sendMotionDownsample(self, value):
        if self.isConnected:
            if value  != '':
                self.motionDownsample = int(float(value))
    ...
    def sendPassiveTorqueFollowDeadzone(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowDeadzoneDeg = max(0.0, float(targetvalue))
            self.setCommand('ZH', str(targetvalue))

    def sendPassiveTorqueRunningSpeedThreshold(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueRunningSpeedThresholdRadS = max(0.1, float(targetvalue))
            self.setCommand('ZR', str(targetvalue))
    ...
    def sendPassiveTorqueFollowPidP(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowLowPidP = float(targetvalue)
                self.passiveTorqueFollowPidP = self.passiveTorqueFollowLowPidP
    ...
    def sendPassiveTorqueFollowPidI(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowLowPidI = float(targetvalue)
                self.passiveTorqueFollowPidI = self.passiveTorqueFollowLowPidI
    ...
    def sendPassiveTorqueFollowPidD(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowLowPidD = float(targetvalue)
                self.passiveTorqueFollowPidD = self.passiveTorqueFollowLowPidD
            self.setCommand('ZD', str(targetvalue))

    def sendPassiveTorqueFollowRunPidP(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowRunPidP = float(targetvalue)
            self.setCommand('ZJ', str(targetvalue))

    def sendPassiveTorqueFollowRunPidI(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowRunPidI = float(targetvalue)
            self.setCommand('ZK', str(targetvalue))

    def sendPassiveTorqueFollowRunPidD(self, targetvalue):
        if self.isConnected:
            if targetvalue != '':
                self.passiveTorqueFollowRunPidD = float(targetvalue)
            self.setCommand('ZL', str(targetvalue))

    def sendPassiveTorqueFieldOffset(self, targetvalue):
        if self.isConnected:
            self.setCommand('ZO', str(targetvalue))
    ...
    def _flushSerialInputBuffer(self):
        if not self.isConnected or self.serialPort is None:
            return
        try:
            with self.commandIoLock:
                self.serialPort.reset_input_buffer()
        except (SerialException, SerialTimeoutException, AttributeError):
            logging.warning('Failed to reset serial input buffer', exc_info=True)
    ...
    def pullConfiguration(self):
        ...
        with self.pullConfigurationLock:
            was_updating_states = self.stateUpdater is not None and not self.stateUpdater.stopped()
            saved_monitor_downsample = self.monitorDownsample
            saved_monitor_variables = list(self.monitorVariables) if isinstance(self.monitorVariables, list) else self.monitorVariables
            restore_monitor_stream = bool(saved_monitor_downsample) and isinstance(saved_monitor_variables, list) and any(saved_monitor_variables)
            self.isPullingConfiguration = True
            if was_updating_states:
                self.stateUpdater.stop()
                self.stateUpdater.wait(500)
                time.sleep(20 / 1000)

            if restore_monitor_stream:
                self.sendMonitorDownsample(0)
                time.sleep(10 / 1000)
                self.sendMonitorClearVariables()
                time.sleep(20 / 1000)

            self._flushSerialInputBuffer()
            ...
            try:
                ...
                time.sleep(5 / 1000)
                self.sendModulationType('')
                time.sleep(5 / 1000)
                self.sendReleaseMode('')
                time.sleep(5 / 1000)
                self.sendPassiveTorqueMode('')
                time.sleep(5 / 1000)
                ...
                time.sleep(5 / 1000)
                self.sendPassiveTorqueRunningSpeedThreshold('')
                time.sleep(5 / 1000)
                ...
                time.sleep(5 / 1000)
                self.sendPassiveTorqueFollowRunPidP('')
                time.sleep(5 / 1000)
                self.sendPassiveTorqueFollowRunPidI('')
                time.sleep(5 / 1000)
                self.sendPassiveTorqueFollowRunPidD('')
                time.sleep(5 / 1000)
                self.sendDeviceStatus('')
            finally:
                if restore_monitor_stream and self.isConnected:
                    time.sleep(10 / 1000)
                    self.sendMonitorDownsample(saved_monitor_downsample)
                    time.sleep(10 / 1000)
                    self.sendMonitorVariables(saved_monitor_variables)
                self.isPullingConfiguration = False
                if was_updating_states and self.isConnected:
                    self.stateUpdater = StateUpdateRunner(self)
                    self.stateUpdater.start()
    ...
    def parseMotionResponse(self, comandResponse):
        if 'downsample' in comandResponse:
            self.motionDownsample = int(float(comandResponse.replace('downsample:', '')))
    ...
    def parseReleaseResponse(self, comandResponse):
    ...
    def parsePassiveTorqueFollowDeadzoneResponse(self, comandResponse):
        self.passiveTorqueFollowDeadzoneDeg = float(
            comandResponse.replace('PassiveTorqueFollowDeadzone:', ''))

    def parsePassiveTorqueRunningSpeedThresholdResponse(self, comandResponse):
        self.passiveTorqueRunningSpeedThresholdRadS = float(
            comandResponse.replace('PassiveTorqueRunningSpeedThreshold:', ''))
    ...
    def parsePassiveTorqueFollowRunPidPResponse(self, comandResponse):
        self.passiveTorqueFollowRunPidP = float(
            comandResponse.replace('PassiveTorqueFollowPidRunP:', ''))

    def parsePassiveTorqueFollowRunPidIResponse(self, comandResponse):
        self.passiveTorqueFollowRunPidI = float(
            comandResponse.replace('PassiveTorqueFollowPidRunI:', ''))

    def parsePassiveTorqueFollowRunPidDResponse(self, comandResponse):
        self.passiveTorqueFollowRunPidD = float(
            comandResponse.replace('PassiveTorqueFollowPidRunD:', ''))
    ...
    def parsePWMModResponse(self, comandResponse):
        if 'center' in comandResponse:
            self.modulationCentered = int(float(comandResponse.replace('center:', '')))
    ...
    def parseMonitorResponse(self, comandResponse):
        if 'all' in comandResponse:
            ...
            separator = ';' if ';' in varStr else '\t'
            states = [item.strip() for item in varStr.rstrip().split(separator) if item.strip() != '']
            if len(states) >= 7:
                self.targetNow = float(states[0])
                self.target = self.targetNow
                self.voltageQNow = float(states[1])
                self.voltageDNow = float(states[2])
                self.currentQNow = float(states[3])
                self.currentDNow = float(states[4])
                self.velocityNow = float(states[5])
                self.angleNow = float(states[6])
    ...
    def parseResponses(self, comandResponse):
        if 'PID vel' in comandResponse:
        ...
        elif 'PassiveTorqueRunningSpeedThreshold' in comandResponse:
            self.parsePassiveTorqueRunningSpeedThresholdResponse(comandResponse)
        ...
        elif 'PassiveTorqueFollowPidRunP' in comandResponse:
            self.parsePassiveTorqueFollowRunPidPResponse(comandResponse)
        elif 'PassiveTorqueFollowPidRunI' in comandResponse:
            self.parsePassiveTorqueFollowRunPidIResponse(comandResponse)
        elif 'PassiveTorqueFollowPidRunD' in comandResponse:
            self.parsePassiveTorqueFollowRunPidDResponse(comandResponse)
            ...
            self.parsePassiveTorqueTargetResponse(comandResponse)
        elif 'Torque' in comandResponse:
            ...
            self.deviceStatus = int(float(comandResponse.replace('Status:', '')))
        elif 'Release' in comandResponse:
            self.parseReleaseResponse(comandResponse)
            ...
            while not self.stopped():
                if self.deviceConnector is not None:
                    if self.deviceConnector.commProvider.serialComm.isOpen():
                        ...
                        interval = 1.0 / max(1, self.deviceConnector.stateUpdateRateHz)
